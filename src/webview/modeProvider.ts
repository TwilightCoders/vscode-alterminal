...
export class AnsiModeProvider implements IModeProvider {
  ...
  setMode(bit: number, enabled: boolean): void {
    ...
    Logger.debug(
      `🎛️ Terminal ${this.terminalId}: Mode ${bit} ${enabled ? "enabled" : "disabled"}`,
    );
  }
    ...
    while ((match = modeRegex.exec(data)) !== null) {
      ...
      switch (modeNumber) {
        case "1004": // Focus reporting
          ...
          Logger.debug(
            `Terminal ${this.terminalId}: Focus reporting ${enable ? "enabled" : "disabled"}`,
          );
          break;
        ...
        case "1049": // Alternate screen
          ...
          Logger.debug(
            `Terminal ${this.terminalId}: Alternate screen ${enable ? "entered" : "exited"}`,
          );
          break;
        ...
        default:
          ...
          Logger.debug(
            `Terminal ${this.terminalId}: Unknown mode ${modeNumber} ${enable ? "enabled" : "disabled"}`,
          );
          break;
      }
    }
  ...
  stripModeSequences(data: string): string {
    ...
    return data.replace(
      /\x1b\[\?(1004|1000|1002|1003|1006|1049|2004)[hl]/g,
      "",
    );
  }
  ...
  restoreModes(): void {
    if (!this.terminal || typeof this.terminal.write !== "function") {
      Logger.warn(
        `Cannot restore modes: terminal not available for ${this.terminalId}`,
      );
      return;
    }
    ...
    try {
      ...
      if (this.hasMode(this.MODES.FOCUS_REPORTING)) {
        Logger.debug(
          `Terminal ${this.terminalId}: Restoring focus reporting mode`,
        );
      ...
      Logger.error(
        `Failed to restore terminal modes for ${this.terminalId}:`,
        error,
      );
    }
  }
  ...
  restoreState(modes: number): void {
    ...
    Logger.debug(
      `🎛️ Mode state restored for terminal ${this.terminalId}: ${modes}`,
    );
  }
