...
export class FilePathLinkProvider implements ILinkProvider {
  ...
  private readonly COMBINED_PATTERN =
    /(\b[\.~]?\/[^\s"'`]*(?:\s[^\s"'`]*)*|[a-z0-9_][^\s\/]*\.[a-z0-9]+|[a-zA-Z]:\\[^\s"'`]+)/gi;
  ...
  setupFilePathLinks(): void {
    Logger.debug(
      "🔗 Setting up file path links (combined) for terminal",
      this.terminalId,
    );
  ...
  private _setupWithLinkProvider(): void {
    ...
    try {
      ...
      Logger.error(
        "Failed to register link provider, falling back to matcher:",
        error,
      );
      this._setupWithLinkMatcher();
    }
  ...
  private _setupWithLinkMatcher(): void {
    Logger.debug(
      "🔗 Setting up file path links with registerLinkMatcher fallback",
    );

    if (
      !this.terminal ||
      typeof this.terminal.registerLinkMatcher !== "function"
    ) {
  ...
  private _validateLink(uri: string, callback: Function): void {
    ...
    const isLikelyFile =
      uri.includes(".") || uri.startsWith("/") || uri.startsWith("~");
