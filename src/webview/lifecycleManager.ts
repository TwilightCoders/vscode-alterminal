...
export class TerminalLifecycleManager implements ILifecycleManager {
    ...
    constructor(private terminal: any, private vscode: any, private terminalId: string) {
    ...
    async initialize(): Promise<void> {
        ...
        try {
            ...
            Logger.debug(`🔄 LifecycleManager initialized for terminal ${this.terminalId}`);
            
        } catch (error) {
            Logger.error(`Failed to initialize lifecycle manager for terminal ${this.terminalId}:`, error);
            throw error;
        }
    }
    ...
    markReady(): void {
        if (!this._isReady) {
            ...
            Logger.debug(`✅ LifecycleManager marked ready for terminal ${this.terminalId}`);
        }
    }
    ...
    dispose(): void {
        ...
        try {
            ...
            Logger.debug(`🗑️ LifecycleManager disposed for terminal ${this.terminalId}`);
            
        } catch (error) {
            Logger.error(`Error disposing lifecycle manager for terminal ${this.terminalId}:`, error);
        }
    }
}
