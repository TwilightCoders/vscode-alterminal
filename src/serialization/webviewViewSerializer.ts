...
export class WebviewViewSerializer {
  ...
  private setupWebviewLifecycle() {
    ...
    this._webviewView.onDidChangeVisibility(() => {
      console.log(
        ...
        this._webviewView?.visible ? "VISIBLE" : "HIDDEN",
      );
  ...
  public async handleStateResponse(state: any): Promise<void> {
    ...
    console.log(
      "💾 Received state for saving:",
      state ? `${state.terminals?.length} terminals` : "no state",
    );
  ...
  public sendSavedStateToWebview(): void {
    ...
    if (savedState && savedState.terminals && savedState.terminals.length > 0) {
      console.log(
        "📤 Sending saved state to webview:",
        savedState.terminals.length,
        "terminals",
      );
      this._webviewView.webview.postMessage({
  ...
  public serialize(tabManager: any): PersistedState {
    console.log(
      "💾 WebviewViewSerializer.serialize() - serializing",
      tabManager.terminals.size,
      "terminals",
    );
    ...
    for (const [id, terminal] of tabManager.terminals) {
      ...
      console.log(`💾 Serialized terminal ${id}:`, {
    ...
    console.log("💾 Final serialized state:", {
  ...
  public deserialize(state: PersistedState, tabManager: any): void {
    console.log(
      "🔄 WebviewViewSerializer.deserialize() - This method is reserved for future extension-side deserialization",
    );
    console.log(
      "🔄 Current deserialization happens in webview context via TabManager.restoreFromState()",
    );
  ...
  public loadFromExtensionStorage(
    ...
    try {
      ...
      console.log(
        ...
        state ? `${state.terminals.length} terminals` : "no saved state",
      );
      return state;
    } catch (error) {
