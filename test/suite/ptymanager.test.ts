...
suite("PTY Manager Test Suite", () => {
  test("Should create temp files with correct permissions", async () => {
    ...
    const mockImageData =
      "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChAI9jU77hwAAAABJRU5ErkJggg==";
  ...
  test("Should validate file size limits", () => {
    ...
    assert.ok(
      smallFile.size < 10 * 1024 * 1024,
      "Small files should be acceptable",
    );
    assert.ok(
      mediumFile.size < 10 * 1024 * 1024,
      "Medium files should be acceptable",
    );
    assert.ok(
      largeFile.size > 10 * 1024 * 1024,
      "Large files exceed reasonable limits",
    );
  });
  ...
  test("Should generate unique temp filenames", () => {
    ...
    assert.notStrictEqual(
      tempFileName1,
      tempFileName2,
      "Temp filenames should be unique",
    );
    assert.ok(
      tempFileName1.includes(fileName),
      "Should preserve original filename",
    );
    assert.ok(
      tempFileName2.includes(fileName),
      "Should preserve original filename",
    );
  });
});
