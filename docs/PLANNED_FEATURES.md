...
```json
{
  "alterminal.defaultTabGroups.global": {
    "autoCreate": true,
    "tabs": [
      {
        "name": "Main Terminal",
...
{
  "alterminal.defaultTabGroups.workspace": {
    "autoCreate": true,
    "tabs": [
      {
        "name": "API Server",
...
#### 3. Project-Level Templates (`.alterminal/tabs.json`)
...
- Custom variables: `${alterminal:projectName}`
...
```
> Alterminal: Configure Default Tab Groups
> Alterminal: Save Current Tabs as Template
> Alterminal: Load Tab Group Template
> Alterminal: Reset to Default Tab Groups
> Alterminal: Import Tab Group Configuration
```
...
1. Project-level (`.alterminal/tabs.json`) - Highest priority
...
- Project templates: `.alterminal/tabs.json` (version controlled)
