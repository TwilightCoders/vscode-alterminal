...
You _can_ use TypeScript for webview code. VS Code does not forbid TS—webviews simply receive final JS. Typical approach:
...
```js
const COMBINED =
  /(\b[\.~]?\/[^\s"'`]*(?:\s[^\s"'`]*)*|[a-z0-9_][^\s\/]*\.[a-z0-9]+|[a-zA-Z]:\\[^\s"'`]+)/g;
// CMD: /[^\s"'`]+/g
```
